...
const std::string COPC_UA_Prosys_Layer::scmModeINITMSG = "INITMSG";
const std::string COPC_UA_Prosys_Layer::scmModeINITUSERTEXT = "INITUSERTEXT";
const std::string COPC_UA_Prosys_Layer::scmModeTRIGGER = "TRIGGER";
const std::string COPC_UA_Prosys_Layer::scmTypeALARM = "Alarm";
const std::string COPC_UA_Prosys_Layer::scmTypeEVENT = "Event";

const std::string COPC_UA_Prosys_Layer::scmAlarmTypeBrowsePath = "/Types/0:EventTypes/0:BaseEventType/0:ConditionType/0:AcknowledgeableConditionType/0:AlarmConditionType/%d:";
const std::string COPC_UA_Prosys_Layer::scmEventTypeBrowsePath = "/Types/0:EventTypes/0:BaseEventType/%d:";
const std::string COPC_UA_Prosys_Layer::scmUserTextAttribute = "TEXT0%d";


const std::string COPC_UA_Prosys_Layer::scmAlarmConditionName = "AlarmCondition";

char COPC_UA_Prosys_Layer::smEmptyString[] = "";

COPC_UA_Prosys_Layer::COPC_UA_Prosys_Layer(CComLayer *paUpperLayer, CBaseCommFB *paComFB) :
  COPC_UA_Layer(paUpperLayer, paComFB), mHandler(nullptr), mMemberActionInfo(nullptr) {
}

COPC_UA_Prosys_Layer::~COPC_UA_Prosys_Layer() {
...
EComResponse COPC_UA_Prosys_Layer::openConnection(char *paLayerParameter) {
  ...
  if(nrOfParams == scmNumberOfParameters) {
    std::string mode(scmModeINITMSG);
    mInitType = std::string(scmTypeALARM);
    mHandler = static_cast<COPC_UA_HandlerAbstract*>(&getExtEvHandler<COPC_UA_Local_Handler>());
    std::string typeName = parser[0];
    eRetVal = initOPCUAType(mode, mInitType, typeName);
    if(eRetVal != e_InitOk) {
      DEVLOG_ERROR("[OPC UA A&C LAYER]: Error at initializing OPC UA Type for FB: %s, Error: %s\n", getCommFB()->getInstanceName(), eRetVal);
      return eRetVal; 
    }
    UA_NodeId_clear(&mTypeNodeId);
    std::string pathToInstance("/Objects");
    eRetVal = createOPCUAObject(mInitType, typeName, pathToInstance);
  } else {
    DEVLOG_ERROR("[OPC UA A&C LAYER]: Wrong number of layer parameters for FB: %s, expected: %d\n", getCommFB()->getInstanceName(), scmNumberOfParameters);
  }
  return eRetVal;
}

void COPC_UA_Prosys_Layer::closeConnection() {
  if(mMemberActionInfo) {
    mHandler->uninitializeAction(*mMemberActionInfo);
  }
}

EComResponse COPC_UA_Prosys_Layer::recvData(const void*, unsigned int) {
  return e_ProcessDataOk;
}

EComResponse COPC_UA_Prosys_Layer::sendData(void*, unsigned int) {
  CIEC_ANY **apoDataPorts = getCommFB()->getSDs();
  mLowLimit = static_cast<CIEC_UINT&>(apoDataPorts[1]->unwrap()).getUnsignedValue();
  mLowLowLimit = static_cast<CIEC_UINT&>(apoDataPorts[2]->unwrap()).getUnsignedValue();
  mHighLimit = static_cast<CIEC_UINT&>(apoDataPorts[3]->unwrap()).getUnsignedValue();
  mHighHighLimit = static_cast<CIEC_UINT&>(apoDataPorts[4]->unwrap()).getUnsignedValue();
  size_t currentValue = static_cast<CIEC_UINT&>(apoDataPorts[0]->unwrap()).getUnsignedValue();

  if(mMemberActionInfo) {
    if(mHandler->executeAction(*mMemberActionInfo) != UA_STATUSCODE_GOOD) {
      DEVLOG_ERROR("[OPC UA A&C LAYER]: Send Data failed for FB %s!\n", getCommFB()->getInstanceName()); 
  ...
  UA_UInt16 severity;
  UA_Boolean active;
  if(currentValue <= mLowLowLimit) {
    // 700, active
    severity = 700;
    active = true;
    nextState = LowLow;
  } else if(currentValue <= mLowLimit) {
    // 500, active
    severity = 500;
    active = true;
    nextState = Low;
  } else if(currentValue > mLowLimit && currentValue < mHighLimit) {
    // 500, inactive
    severity = 500;
    active = false;
    nextState = Inactive;
  } else if(currentValue >= mHighLimit && currentValue < mHighHighLimit) {
    // 500, active
    severity = 500;
    active = true;
    nextState = High;
  ...
  if(nextState != currentState) {
    ...
    if(!currentRetainState) {
      setAlarmRetain(server, true); 
      currentRetainState = true;
    }
    if(severity != currentSeverityState) {
      setAlarmSeverity(server, severity);
      currentSeverityState = severity;
    }
    if(active != currentActiveState) {
      setAlarmActive(server, active);
      currentActiveState = active;
    }
    if(triggerAlarm(active) != UA_STATUSCODE_GOOD) {
...
EComResponse COPC_UA_Prosys_Layer::processInterrupt() {
  // TODO
  return e_ProcessDataOk;
}

void COPC_UA_Prosys_Layer::setAlarmRetain(UA_Server *paServer, UA_Boolean paRetain) {
  char *retainProperty = getNameFromString("Retain");
  UA_QualifiedName retainField = UA_QUALIFIEDNAME(0,retainProperty);
  UA_Variant value;
  UA_Variant_setScalar(&value, &paRetain, &UA_TYPES[UA_TYPES_BOOLEAN]);
  UA_StatusCode status = UA_Server_setConditionField(paServer, mConditionInstanceId,
                                                      &value, retainField);
...
void COPC_UA_Prosys_Layer::setAlarmActive(UA_Server *paServer, UA_Boolean paActive) {
  ...
  UA_Variant value;
  UA_Variant_setScalar(&value, &paActive, &UA_TYPES[UA_TYPES_BOOLEAN]);
  UA_StatusCode status = UA_Server_setConditionVariableFieldProperty(paServer, mConditionInstanceId,
...
void COPC_UA_Prosys_Layer::setAlarmSeverity(UA_Server *paServer, UA_UInt16 paSeverity) {
  char *severityProperty = getNameFromString("Severity");
  UA_QualifiedName severityField = UA_QUALIFIEDNAME(0,severityProperty);

  UA_Variant value;
  UA_Variant_setScalar(&value, &paSeverity, &UA_TYPES[UA_TYPES_UINT16]);
  UA_StatusCode status = UA_Server_setConditionField(paServer, mConditionInstanceId,
                                                      &value, severityField);
  if(status != UA_STATUSCODE_GOOD) {
    DEVLOG_ERROR("[OPC UA A&C LAYER]: Writing Severity Property failed, StatusCode: %s\n", UA_StatusCode_name(status));
  }
}

UA_StatusCode COPC_UA_Prosys_Layer::triggerAlarm(UA_Boolean active) {
  COPC_UA_Local_Handler* localHandler = static_cast<COPC_UA_Local_Handler*>(mHandler);
  UA_Server *server = localHandler->getUAServer();
  ...
  if(!active) {
    ...
    if(status != UA_STATUSCODE_GOOD) {
      DEVLOG_ERROR("[OPC UA A&C LAYER]: Triggering Condition Event failed, StatusCode: %s\n", UA_StatusCode_name(status));
      return status;
    }          
  }
  return status;
...
UA_StatusCode COPC_UA_Prosys_Layer::triggerEvent() {
  COPC_UA_Local_Handler* localHandler = static_cast<COPC_UA_Local_Handler*>(mHandler);
  UA_Server *server = localHandler->getUAServer();
  UA_NodeId eventNode;
...
EComResponse COPC_UA_Prosys_Layer::initOPCUAType(const std::string &paMode, const std::string &paType, const std::string &paTypeName) {
  EComResponse eRetVal = e_InitTerminated;
  COPC_UA_Local_Handler* localHandler = static_cast<COPC_UA_Local_Handler*>(mHandler);
...
EComResponse COPC_UA_Prosys_Layer::createOPCUAObject(const std::string &paType, const std::string &paTypeName, const std::string &paPathToInstance) {
...
UA_StatusCode COPC_UA_Prosys_Layer::createOPCUAObjectNode(UA_Server *paServer, const std::string &paPathToInstance, std::string &paBrowsePath) {
...
UA_StatusCode COPC_UA_Prosys_Layer::addOPCUACondition(UA_Server *paServer, const std::string &paPathToInstance, std::string &paBrowsePath) {
...
forte::com_infra::EComResponse COPC_UA_Prosys_Layer::initializeMemberActions(const std::string &paParentBrowsePath) {
...
EComResponse COPC_UA_Prosys_Layer::createAlarmType(UA_Server *paServer, const std::string &paTypeName) {
...
EComResponse COPC_UA_Prosys_Layer::createEventType(UA_Server *paServer, const std::string &paTypeName) {
...
EComResponse COPC_UA_Prosys_Layer::addOPCUATypeProperties(UA_Server *paServer, const std::string &paMode, const std::string &paTypeName) {
  EComResponse eRetVal = e_InitTerminated;
  bool isPublisher;
...
forte::com_infra::EComResponse COPC_UA_Prosys_Layer::addOPCUATypeMSGProperties(UA_Server *paServer, const std::string &paParentTypeName, bool paIsPublisher) {
...
forte::com_infra::EComResponse COPC_UA_Prosys_Layer::addOPCUATypeUSERProperties(UA_Server *paServer, const std::string &paParentTypeName, bool paIsPublisher) {
  CIEC_ANY **apoDataPorts = paIsPublisher ? getCommFB()->getSDs() : getCommFB()->getRDs();
  size_t numDataPorts = paIsPublisher ? getCommFB()->getNumSD() : getCommFB()->getNumRD();
  for(size_t i = 0; i < numDataPorts; i++) {
...
UA_StatusCode COPC_UA_Prosys_Layer::addVariableNode(UA_Server *paServer, const std::string &paParentTypeName, char *paVariableName, CIEC_ANY &paVariableType) {
...
bool COPC_UA_Prosys_Layer::isOPCUAObjectPresent(std::string &paBrowsePath) {
  COPC_UA_Local_Handler* localHandler = static_cast<COPC_UA_Local_Handler*>(mHandler);
  if(localHandler) {
...
std::string COPC_UA_Prosys_Layer::getPortNameFromConnection(CStringDictionary::TStringId paPortNameId, bool paIsPublisher) {
...
std::string COPC_UA_Prosys_Layer::getFBNameFromConnection() {
  bool isPublisher;
...
char *COPC_UA_Prosys_Layer::getNameFromString(const std::string &paName) {
