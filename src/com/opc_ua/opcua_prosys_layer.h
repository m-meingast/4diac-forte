...
class COPC_UA_Prosys_Layer : public COPC_UA_Layer {
  public:

    COPC_UA_Prosys_Layer(forte::com_infra::CComLayer *paUpperLayer, forte::com_infra::CBaseCommFB *paComFB);

    ~COPC_UA_Prosys_Layer() override;
  ...
  private:
    ...
    size_t mLowLowLimit;
    size_t mHighLimit;
    size_t mHighHighLimit;

    State currentState = Init;
    UA_Boolean currentRetainState = false;
    UA_Boolean currentActiveState = false;
    UA_UInt16 currentSeverityState = 1;
    ...
    UA_StatusCode triggerAlarm(UA_Boolean active);

    UA_StatusCode triggerEvent();

    void setAlarmRetain(UA_Server *paServer, UA_Boolean paRetain);

    void setAlarmActive(UA_Server *paServer, UA_Boolean paActive);

    void setAlarmSeverity(UA_Server *paServer, UA_UInt16 paSeverity);
